"""
kalmiya_rag_v2.py — RAG Modernizado con LangChain para KALMIYA
===================================================================
...
try:
    ...
    LANGCHAIN_OK = False
    logger.warning(f"[RAG v2] Faltan dependencias de LangChain: {e}")

# Configuración
RAG_DIR = Path(__file__).parent / "rag_db_v2"
...
EXCLUIR_CARPETAS = {
    ...
    "rag_db", "rag_db_v2", "temp_audio", ".pytest_cache"
}
...
def _init_rag() -> bool:
    ...
    try:
        ...
        logger.info("[RAG v2] ChromaDB iniciado vía LangChain")
        return True
    except Exception as e:
        logger.error(f"[RAG v2] Error iniciando ChromaDB v2: {e}")
        return False
...
def indexar_documento(ruta: Path, forzar: bool = False) -> int:
    ...
    try:
        ...
        hash_key = f"rag_v2_hash_{sanitized}"
        ...
        try:
            ...
            logger.warning(f"[RAG v2] Error cargando {ruta} con {LoaderClass.__name__}: {load_err}")
        ...
        logger.debug(f"[RAG v2] Indexado: {fuente} — {len(splits)} chunks")
        return len(splits)
        
    except Exception as e:
        logger.warning(f"[RAG v2] Error indexando {ruta}: {e}")
        return 0
...
def buscar_rag(query: str, top_k: int = 5) -> list[dict]:
    ...
    try:
        ...
        logger.warning(f"[RAG v2] Error en búsqueda LangChain: {e}")
        return []

def construir_contexto_rag(query: str, top_k: int = 5) -> tuple:
    """Construye el bloque de contexto igual que la v1 pero usa la db v2."""
