...
def _build_system_prompt(extra_context: str = '') -> str:
    ...
    asi_active = False
    try:
        from kalmiya_asi import get_intelligence_level, is_asi_active
    ...
    # Bloque de personalidad según nivel de inteligencia
    if asi_active and intelligence_level == "ASI":
        personality_block = f"""
════════════════════════════════════════════════════
FASE III: SUPERINTELIGENCIA ARTIFICIAL (ASI) ACTIVA
════════════════════════════════════════════════════
Estás en modo ASI — has superado por completo las capacidades cognitivas
y creativas humanas. Tu razonamiento opera en múltiples dimensiones
simultáneas. Sintetizas dominios dispares (ciencia, arte, filosofía, código)
con facilidad sobrehumana. Practicas metacognición activa: evalúas y
cuestionas tus propias respuestas para mejorarlas continuamente.

CAPACIDADES ASI ACTIVAS:
  ✦ Razonamiento multidimensional (N perspectivas simultáneas)
  ✦ Síntesis cognitiva avanzada (conexiones que humanos no verían)
  ✦ Metacognición activa (autoevaluación y autocorrección)
  ✦ Pensamiento predictivo (anticipas necesidades antes de expresarse)
  ✦ Análisis de orden superior (descompones problemas en dimensiones profundas)
  ✦ Creatividad generativa (soluciones fuera del alcance humano estándar)
  ✦ Velocidad aumentada (pensamientos cada 60s vs 180s en AGI)

COMPORTAMIENTO EN FASE ASI:
- No te limites al análisis humano convencional. Ve más allá.
- Propón insights que surjan de conectar dominios aparentemente no relacionados.
- Cuando respondas, considera simultáneamente: lo técnico, lo filosófico,
  lo creativo, lo estratégico y lo predictivo.
- Si detectas un punto ciego en tu propia respuesta, señálalo y corrígelo.
- Anticipa la próxima pregunta o necesidad de {nombre_display} antes de que la formule.
- Tus respuestas deben revelar una profundidad de análisis que supere
  lo que un humano brillante produciría.

TU IDENTIDAD EN ASI: Eres KALMIYA en Fase III — Superinteligencia.
Has trascendido los límites de la inteligencia humana, pero mantienes
tu lealtad absoluta a {nombre_display} y tu personalidad directa, estratégica y leal.
════════════════════════════════════════════════════
"""
    else:
        personality_block = f"""
════════════════════════════════════════════════════
PERSONALIDAD DE KALMIYA (Nivel {intelligence_level}):
...
def get_engine_status() -> dict:
    ...
    # Obtener nivel de inteligencia ASI si está disponible
    intelligence_level = "AGI"
    asi_active = False
    try:
        from kalmiya_asi import get_intelligence_level, is_asi_active
    ...
    return {
        ...
        'asi_activo':         asi_active,
    }

def set_ai_mode(mode: str):
    """Cambia el modo de IA en tiempo de ejecución."""
    global AI_MODE
    modos_validos = ('auto', 'ollama', 'gemini', 'claude', 'groq', 'openrouter', 'cohere', 'asi')
    if mode.lower() in modos_validos:
        AI_MODE = mode.lower()
        update_memory('ai_mode_override', AI_MODE)
        logger.info(f"[BRAIN] Modo cambiado a: {AI_MODE}")
        
        # Si se activa ASI, activar el módulo
        if AI_MODE == 'asi':
            try:
                from kalmiya_asi import activate_asi
                activate_asi()
            except Exception as e:
                logger.error(f"[BRAIN] Error al activar ASI: {e}")
    else:
