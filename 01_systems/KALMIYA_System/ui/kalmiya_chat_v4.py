...
class KalmiyaChatV4:
    """Chat KALMIYA V4.0 - Todas las características"""
    ...
    def __init__(self):
        ...
        self._running = True
        self._current_theme = "cyber_cyan"
    ...
    def _build_avatar_animated(self):
        ...
        self.wave_canvas.pack(anchor="w", pady=(5,0))
        self._anim_frame = 0
    ...
    def _draw_wave(self, idle=True):
        ...
        if idle:
            self.wave_canvas.create_line(10, 15, 270, 15, fill=self.theme["text_gray"], width=2)
        else:
            import math
            points = []
            for i in range(0, 280, 5):
                y = 15 + math.sin((i + self._anim_frame) * 0.2) * 8
                points.extend([i, y])
            if len(points) >= 4:
                self.wave_canvas.create_line(points, fill=self.theme["accent_primary"], width=3, smooth=True)
