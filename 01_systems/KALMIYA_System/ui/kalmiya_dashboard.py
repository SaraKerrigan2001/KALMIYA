...
def _resolve_obsidian_note_path(relative_path: str) -> str | None:
    """Resuelve un path de nota dentro del vault, usando .md si hace falta."""
    vault = get_vault()
    candidate = vault / relative_path
    if candidate.exists():
        return str(candidate.relative_to(vault)).replace('\\', '/')

    if candidate.suffix == '':
        candidate_md = candidate.with_suffix('.md')
        if candidate_md.exists():
            return str(candidate_md.relative_to(vault)).replace('\\', '/')

    return None


def _format_note_link(relative_path: str, alias: str | None = None) -> str:
    """Devuelve un enlace wiki válido para una nota si existe."""
    resolved = _resolve_obsidian_note_path(relative_path)
    if not resolved:
        return f"{relative_path}"
    wiki_path = resolved[:-3] if resolved.endswith('.md') else resolved
    if alias:
        return f"[[{wiki_path}|{alias}]]"
    return f"[[{wiki_path}]]"


def _list_root_notes() -> list[str]:
    """Lista las notas .md que están en la raíz del vault."""
    vault = get_vault()
    notes = []
    for path in sorted(vault.glob('*.md')):
        if path.name == DASHBOARD_FILE:
            continue
...
def _should_ignore_path(path: Path) -> bool:
    """Devuelve True si la ruta debe ser ignorada al buscar notas."""
    ignore_dirs = {'.venv', '.venv-1', '__pycache__', '.git', 'node_modules'}
    return any(part in ignore_dirs for part in path.parts)


def _list_section_notes(base_folder: str, max_depth: int = 2) -> list[tuple[str, str]]:
    """Lista notas dentro de una carpeta de proyecto con alias limpios."""
    vault = get_vault()
    ...
    for path in sorted(folder.rglob('*.md')):
        if _should_ignore_path(path):
            continue
        if len(path.relative_to(vault).parts) > max_depth + len(Path(base_folder).parts):
            continue
        rel_path = str(path.relative_to(vault)).replace('\\', '/')
        alias = path.stem
        notes.append((rel_path, alias))
    return notes


def _canonical_generic_notes() -> list[tuple[str, str]]:
    """Devuelve una lista canónica de notas genéricas importantes."""
    candidates = [
        ("README.md", "README raíz"),
        ("INDEX.md", "INDEX raíz"),
        ("01_systems/KALMIYA_System/README.md", "README KALMIYA System"),
        ("01_systems/KALMIYA/README.md", "README KALMIYA"),
        ("01_systems/LLM_Wiki/README.md", "README LLM Wiki"),
        ("01_systems/LLM_Wiki/schema/SCHEMA.md", "SCHEMA LLM Wiki"),
        ("01_systems/LLM_Wiki/wiki/index.md", "Índice LLM Wiki"),
        ("01_systems/LLM_Wiki/wiki/log.md", "Log LLM Wiki"),
    ]
    notes: list[tuple[str, str]] = []
    for rel_path, alias in candidates:
        resolved = _resolve_obsidian_note_path(rel_path)
        if resolved:
            notes.append((resolved, alias))
    return notes
...
def generate_dashboard() -> str:
    ...
    root_notes = _list_root_notes()
    if root_notes:
        nav_links = [f"[[{Path(note).stem}]]" for note in root_notes]
        lines.append(" | ".join(nav_links))
    else:
        lines.append("*No se encontraron notas raíz en el vault.*")
    lines += [
        "",
        "---",
        "",
    ...
    if root_notes:
        for note in root_notes:
            lines.append(f"- [[{Path(note).stem}]]")
    else:
        lines.append("*No hay documentos raíz para conectar.*")
    canonical_notes = _canonical_generic_notes()
    if canonical_notes:
        lines += [
            "",
        ...
        for rel_path, alias in canonical_notes:
            lines.append(f"- {_format_note_link(rel_path, alias)}")
    lines += [
        "",
        "### Notas de sistema y docs técnicos",
        "",
    ]
    for rel_path, alias in _list_section_notes("01_systems/KALMIYA_System", max_depth=2):
        lines.append(f"- {_format_note_link(rel_path, alias)}")
    for rel_path, alias in _list_section_notes("01_systems/KALMIYA", max_depth=2):
        lines.append(f"- {_format_note_link(rel_path, alias)}")
    for rel_path, alias in _list_section_notes("01_systems/LLM_Wiki", max_depth=3):
        lines.append(f"- {_format_note_link(rel_path, alias)}")
    lines += [
        "",
    ...
    for rel_path, alias in _canonical_generic_notes():
        lines.append(f"- {_format_note_link(rel_path, alias)}")
    lines += [
        "",
        "### Notas KALMIYA",
    ...
    for rel_path, alias in _list_section_notes("KALMIYA_Notes", max_depth=2):
        lines.append(f"- {_format_note_link(rel_path, alias)}")
    lines += [
        "",
    ...
    extra_notes = [
        ...
        ("OneDrive_Docs/GitHub/proyecto_elementos/README.md", "GitHub proyecto README"),
        ("OneDrive_Docs/GitHub/proyecto_elementos/CHECKLIST_GITHUB.md", "Checklist GitHub"),
        ("OneDrive_Docs/GitHub/proyecto_elementos/INSTRUCCIONES_GITHUB.md", "Instrucciones GitHub"),
        ("OneDrive_Docs/GitHub/proyecto_elementos/examples/README.md", "Ejemplos GitHub README"),
        ("02_infrastructure/Lib/README.md", "Infra Lib README"),
        ("02_infrastructure/reports/README.md", "Infra Reports README"),
        ("02_infrastructure/scratch/README.md", "Infra Scratch README"),
        ("02_infrastructure/Scripts/README.md", "Infra Scripts README"),
    ]
    for rel_path, alias in extra_notes:
        lines.append(f"- {_format_note_link(rel_path, alias)}")
    lines += [
        "",
    ]
    ...
    note_definitions = [
        ...
        ("FUNCIONES_IMPLEMENTACION", None, "Estado de implementación"),
    ]

    for note_path, alias, description in note_definitions:
        note_link = _format_note_link(note_path, alias)
        lines.append(f"- {note_link} — {description}")
