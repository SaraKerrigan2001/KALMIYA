...
# NOTA: THOUGHT_INTERVAL ahora es dinamico y se obtiene desde kalmiya_asi.get_intelligence_info()
THOUGHT_INTERVAL   = 180   # Default AGI: Pensar cada 3 minutos (ANI=300s, ASI=60s)
...
def _generate_autonomous_thought(startup: bool = False):
    ...
    En modo ASI, genera pensamientos más profundos usando generate_asi_thought().
    """
    if not (is_ollama_running() or is_gemini_configured()):
        return

    try:
        # Verificar si ASI está activo
        asi_active = False
        try:
            from kalmiya_asi import is_asi_active, generate_asi_thought
        ...
        # Si ASI está activo, usar generate_asi_thought() como prompt
        if asi_active and not startup:
            prompt = generate_asi_thought()
        else:
            # Tipos de pensamientos autonomos normales (AGI/ANI)
            thought_types = [
...
def _autonomous_loop():
    ...
    Intervalo de pensamientos ahora es dinámico según nivel de inteligencia.
    """
    ...
    while _core_running:
        try:
            ...
            # Obtener intervalo dinámico desde kalmiya_asi si está disponible
            try:
                from kalmiya_asi import get_intelligence_info
                _current_thought_interval = get_intelligence_info()['thought_interval']
            except Exception:
                with _thought_interval_lock:
